...
const port = process.env.PORT ||3000;           // constante que tiene el port que vamos a utilizar, es recomendable establecer que lo maneje una variable de entorno
...
app.get('/api', (req,res) => {                  // asi se crea una nueva page donde res(response) es la info que se trae del servidor y req(require) es la informacion que se manda al servidor
  res.send('Hola mi server en express');    //  se utilisa res.send() para enviar algo a la url designada
});

app.get('/api/nueva-ruta', (req,res) => {   //creando nueva ruta
